...
namespace Latvian.PartOfSpeechTagging.Evaluation
{
    public class CrossValidation<T> where T : ITrainedTagger
    {
        ...
        public Results Evaluate()
        {
            ...
            Parallel.For(0, Folds, (i) =>
            {
                ...
                if (tagger is PartOfSpeechTagging.Perceptron.PerceptronTagger)
                    ((PartOfSpeechTagging.Perceptron.PerceptronTagger)tagger).IterationStarted += (it) => { lock (folds) progress++; System.Diagnostics.Debug.WriteLine(string.Format("Fold {0} iteration {1}, {2:0}% done", i, it, 100 * ((double)progress / (Folds * Folds)))); };
